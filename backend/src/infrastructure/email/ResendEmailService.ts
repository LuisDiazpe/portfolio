...
export class ResendEmailService implements EmailService {
    ...
    async send(msg: ContactMessage): Promise<{ messageId: string }> {
        ...
        const toEmail   = process.env.CONTACT_RECEIVER ?? 'luidi678u@gmail.com'

        try {
            //Correo hacia mi (Notificación de nuevo contacto)
            ...
            if (autoReply.error) {
                console.warn('[Resend] Falló la auto-respuesta:', autoReply.error.message)
            }

            return { messageId: notification.data?.id ?? 'unknown' }
        } catch (error) {
            console.error('[ResendEmailService] Error crítico:', error)
            throw error
        }
    }
