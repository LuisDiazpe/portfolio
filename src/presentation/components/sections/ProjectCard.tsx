...
const STATUS_CLASS: Record<Project['status'], string> = {
  production:    'bg-aurora-cyan/10 text-aurora-cyan border-aurora-cyan/20',
...
export function ProjectCard({ project, index }: ProjectCardProps) {
  ...
  // Get translated title/description if language is not spanish
  const pd = lang !== 'es'
      ? t(`projects_data.${project.id}`, { returnObjects: true }) as Record<string, string> | string
      : null
  const tTitle = (pd && typeof pd === 'object' && 'title' in pd) ? pd.title : project.title
  const tDesc  = (pd && typeof pd === 'object' && 'description' in pd) ? pd.description : project.description
  ...
  return (
      ...
      >
        ...
        <div className="flex flex-col flex-1 p-5">
          ...
          <span className={`text-xs px-2.5 py-1 rounded-full border font-medium ${STATUS_CLASS[project.status]}`}>
