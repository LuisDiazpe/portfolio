...
import { ArrowDown, Github, Linkedin, Gitlab, ExternalLink } from 'lucide-react'
import { lazy, Suspense } from 'react'

// Lazy load 3D — avoids blocking initial render
        ...
        <div className="max-w-6xl mx-auto w-full flex flex-col lg:flex-row lg:items-center lg:gap-8">
          ...
          >
            ...
            <motion.p
                ...
                className="text-text-muted text-base md:text-lg max-w-lg mb-8 leading-relaxed"
            >
            ...
            <motion.div variants={stagger.item} className="flex flex-wrap gap-3 mb-14">
              ...
              <button
                ...
                transition-colors focus-aurora"
              >
                Hablemos
              </button>
              <a
                ...
                border border-border hover:border-aurora-cyan/30 hover:text-text-primary
                transition-all focus-aurora"
              >
                <Github size={16} />
              </a>
              <a
                  href="https://gitlab.com/LuisDiazpe"
                  target="_blank" rel="noreferrer" aria-label="GitLab"
                  className="flex items-center px-4 py-3 rounded-xl text-sm text-text-muted
                border border-border hover:border-orange-400/30 hover:text-orange-400
                transition-all focus-aurora"
              >
                <Gitlab size={16} />
              </a>
              <a
                  href="https://www.linkedin.com/in/luis-d%C3%ADaz-b2b537293/"
                  target="_blank" rel="noreferrer" aria-label="LinkedIn"
                  className="flex items-center px-4 py-3 rounded-xl text-sm text-text-muted
                border border-border hover:border-aurora-blue/30 hover:text-aurora-blue
                transition-all focus-aurora"
              >
                <Linkedin size={16} />
              </a>
            ...
            {/* Stats — separated with border-t for breathing room */}
            <motion.div
                variants={stagger.item}
                className="flex flex-wrap gap-8 md:gap-12 pt-6 border-t border-border/30"
            >
              {STATS.map((s, i) => (
                  <motion.div
                      key={s.label}
          ...
          >
            <Suspense
                fallback={
                  <div className="w-full h-full rounded-2xl border border-border/20
                bg-bg-card/10 flex items-center justify-center">
                    <div className="flex flex-col items-center gap-3 opacity-40">
                      <div className="w-8 h-8 border-2 border-aurora-cyan/40 border-t-aurora-cyan
                    rounded-full animate-spin" />
                      <span className="text-xs font-mono text-text-subtle">Cargando modelo...</span>
                    </div>
                  </div>
                }
            >
